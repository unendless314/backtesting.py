...
**持有期間**: 365 天
**分析進場區間**: 2018-11-06 至 2025-09-08

## 統計概覽 (Statistics Overview)
| 指標 | 數值 | 說明 |
| :--- | :--- | :--- |
| **觸發交易天數** | 660 天 | 佔總樣本數的 26.4% |
| **勝率 (Win Rate)** | **71.52%** (472 天) | 買入持有 365 天後獲利的機率 |
| **賠率 (Loss Rate)** | 28.48% (188 天) | 買入持有 365 天後虧損的機率 |
| **偏度 (Skewness)** | 1.67 | >1 代表具有「暴漲長尾」特性 |

## 回報率分析 (Returns Analysis)
| 指標 | 平均值 (Mean) | 中位數 (Robust) |
| :--- | :--- | :--- |
| **整體回報率** | 593.06% | **89.08%** |
| **獲利交易平均回報** | +836.36% | +163.75% |
| **虧損交易平均回報** | -17.80% | -20.69% |
| **盈虧比 (Reward/Risk)** | **47.00** | **7.91** |

## 情景分析 (Scenario Analysis - Based on Quartiles)
| 情景 (Scenario) | 機率分界 (Percentile) | 預期回報率 (ROI) |
| :--- | :--- | :--- |
| **樂觀情景 (Optimistic)** | Top 25% (Q3) | > **212.90%** |
| **基準情景 (Baseline)** | Median (Q2) | **89.08%** |
| **悲觀情景 (Pessimistic)** | Bottom 25% (Q1) | < **-4.33%** |
...
| :--- | :--- | :--- |
| **標準策略 (基於平均值)** | `70.91%` | `35.45%` |
| **穩健策略 (基於中位數)** | `67.92%` | `33.96%` |
| **保守策略 (基於保守預期)** | `62.63%` | `31.31%` |
| **激進策略 (基於樂觀預期)** | `71.33%` | `(不建議)` |
