...
**分析進場區間**: 2017-11-06 至 2025-09-08

## 統計概覽 (Statistics Overview)
| 指標 | 數值 | 說明 |
| :--- | :--- | :--- |
| **總交易樣本數** | 2864 天 | 模擬在此區間內每天買入 |
| **勝率 (Win Rate)** | **65.96%** (1889 天) | 持有 365 天後獲利的機率 |
| **賠率 (Loss Rate)** | 34.04% (975 天) | 持有 365 天後虧損的機率 |
| **偏度 (Skewness)** | 3.02 | >1 代表具有「暴漲長尾」特性 |

## 回報率分析 (Returns Analysis)
| 指標 | 平均值 (Mean) | 中位數 (Robust) |
| :--- | :--- | :--- |
| **整體回報率** | 267.28% | **37.52%** |
| **獲利交易平均回報** | +418.55% | +103.16% |
| **虧損交易平均回報** | -25.79% | -23.46% |
| **盈虧比 (Reward/Risk)** | **16.23** | **4.40** |

## 情景分析 (Scenario Analysis - Based on Quartiles)
| 情景 (Scenario) | 機率分界 (Percentile) | 預期回報率 (ROI) |
| :--- | :--- | :--- |
| **樂觀情景 (Optimistic)** | Top 25% (Q3) | > **139.78%** |
| **基準情景 (Baseline)** | Median (Q2) | **37.52%** |
| **悲觀情景 (Pessimistic)** | Bottom 25% (Q1) | < **-15.78%** |
...
| :--- | :--- | :--- |
| **標準策略 (基於平均值)** | `63.86%` | `31.93%` |
| **穩健策略 (基於中位數)** | `58.22%` | `29.11%` |
| **保守策略 (基於保守預期)** | `36.15%` | `18.07%` |
| **激進策略 (基於樂觀預期)** | `63.12%` | `(不建議)` |
