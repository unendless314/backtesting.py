...
**數據來源**: `ETHUSDT_1d.csv`
**策略條件**: 當「過去一年報酬率」低於 `-34.81%` 時買入
**持有期間**: 365 天
**分析進場區間**: 2018-08-17 至 2025-09-09

## 統計概覽 (Statistics Overview)
| 指標 | 數值 | 說明 |
| :--- | :--- | :--- |
| **觸發交易天數** | 663 天 | 佔總樣本數的 25.7% |
| **勝率 (Win Rate)** | **76.92%** (510 天) | 買入持有 365 天後獲利的機率 |
| **賠率 (Loss Rate)** | 23.08% (153 天) | 買入持有 365 天後虧損的機率 |
| **偏度 (Skewness)** | 0.58 | >1 代表具有「暴漲長尾」特性 |

## 回報率分析 (Returns Analysis)
| 指標 | 平均值 (Mean) | 中位數 (Robust) |
| :--- | :--- | :--- |
| **整體回報率** | 37.58% | **28.65%** |
| **獲利交易平均回報** | +51.93% | +49.32% |
| **虧損交易平均回報** | -10.25% | -8.83% |
| **盈虧比 (Reward/Risk)** | **5.07** | **5.58** |

## 情景分析 (Scenario Analysis - Based on Quartiles)
| 情景 (Scenario) | 機率分界 (Percentile) | 預期回報率 (ROI) |
| :--- | :--- | :--- |
| **樂觀情景 (Optimistic)** | Top 25% (Q3) | > **70.60%** |
| **基準情景 (Baseline)** | Median (Q2) | **28.65%** |
| **悲觀情景 (Pessimistic)** | Bottom 25% (Q1) | < **1.43%** |
...
| :--- | :--- | :--- |
| **標準策略 (基於平均值)** | `72.37%` | `36.18%` |
| **穩健策略 (基於中位數)** | `72.79%` | `36.40%` |
| **保守策略 (基於保守預期)** | `60.44%` | `30.22%` |
| **激進策略 (基於樂觀預期)** | `75.64%` | `(不建議)` |
