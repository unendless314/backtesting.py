...
**數據來源**: `VOO_1d.csv`
**策略條件**: 當「過去一年報酬率」低於 `-10.00%` 時買入
**持有期間**: 252 天
**分析進場區間**: 2011-09-08 至 2025-09-05

## 統計概覽 (Statistics Overview)
| 指標 | 數值 | 說明 |
| :--- | :--- | :--- |
| **觸發交易天數** | 151 天 | 佔總樣本數的 4.3% |
| **勝率 (Win Rate)** | **100.00%** (151 天) | 買入持有 252 天後獲利的機率 |
| **賠率 (Loss Rate)** | 0.00% (0 天) | 買入持有 252 天後虧損的機率 |
| **偏度 (Skewness)** | 2.33 | >1 代表具有「暴漲長尾」特性 |

## 回報率分析 (Returns Analysis)
| 指標 | 平均值 (Mean) | 中位數 (Robust) |
| :--- | :--- | :--- |
| **整體回報率** | 21.99% | **17.64%** |
| **獲利交易平均回報** | +21.99% | +17.64% |
| **虧損交易平均回報** | -0.00% | -0.00% |
| **盈虧比 (Reward/Risk)** | **∞ (無虧損)** | **∞ (無虧損)** |

## 情景分析 (Scenario Analysis - Based on Quartiles)
| 情景 (Scenario) | 機率分界 (Percentile) | 預期回報率 (ROI) |
| :--- | :--- | :--- |
| **樂觀情景 (Optimistic)** | Top 25% (Q3) | > **23.21%** |
| **基準情景 (Baseline)** | Median (Q2) | **17.64%** |
| **悲觀情景 (Pessimistic)** | Bottom 25% (Q1) | < **14.32%** |
...
| :--- | :--- | :--- |
| **標準策略 (基於平均值)** | `100.00%` | `50.00%` |
| **穩健策略 (基於中位數)** | `100.00%` | `50.00%` |
| **保守策略 (基於保守預期)** | `100.00%` | `50.00%` |
| **激進策略 (基於樂觀預期)** | `100.00%` | `(不建議)` |
