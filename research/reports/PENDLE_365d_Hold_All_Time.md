...
**完整數據範圍**: 2021-04-29 至 2026-09-07
**分析進場區間**: 2021-04-29 至 2025-09-07

## 統計概覽 (Statistics Overview)
| 指標 | 數值 | 說明 |
| :--- | :--- | :--- |
| **總交易樣本數** | 1593 天 | 模擬在此區間內每天買入 |
| **勝率 (Win Rate)** | **51.10%** (814 天) | 持有 365 天後獲利的機率 |
| **賠率 (Loss Rate)** | 48.90% (779 天) | 持有 365 天後虧損的機率 |
| **偏度 (Skewness)** | 1.79 | >1 代表具有「暴漲長尾」特性 |
...
| **虧損交易平均回報** | -62.80% | -63.99% |
| **盈虧比 (Reward/Risk)** | **13.95** | **9.73** |

## 情景分析 (Scenario Analysis - Based on Quartiles)
| 情景 (Scenario) | 機率分界 (Percentile) | 預期回報率 (ROI) |
| :--- | :--- | :--- |
| **樂觀情景 (Optimistic)** | Top 25% (Q3) | > **670.47%** |
| **基準情景 (Baseline)** | Median (Q2) | **7.42%** |
| **悲觀情景 (Pessimistic)** | Bottom 25% (Q1) | < **-63.42%** |
...
| :--- | :--- | :--- |
| **標準策略 (基於平均值)** | `47.59%` | `23.80%` |
| **穩健策略 (基於中位數)** | `46.07%` | `23.04%` |
| **保守策略 (基於保守預期)** | `36.20%` | `18.10%` |
| **激進策略 (基於樂觀預期)** | `49.30%` | `(不建議)` |
