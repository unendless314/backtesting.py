# ETH 逢低買入策略 (Dip Buy Strategy)

**數據來源**: `ETHUSDT_1d.csv`
**策略條件**: 當「過去一年報酬率」低於 `-33.58%` 時買入
**持有期間**: 365 天
**分析進場區間**: 2018-08-17 至 2025-01-22

## 統計概覽 (Statistics Overview)
| 指標 | 數值 | 說明 |
| :--- | :--- | :--- |
| **觸發交易天數** | 609 天 | 佔總樣本數的 25.9% |
| **勝率 (Win Rate)** | **75.86%** (462 天) | 買入持有 365 天後獲利的機率 |
| **賠率 (Loss Rate)** | 24.14% (147 天) | 買入持有 365 天後虧損的機率 |
...
| :--- | :--- | :--- |
| **整體回報率** | 39.69% | **32.24%** |
| **獲利交易平均回報** | +55.58% | +59.23% |
| **虧損交易平均回報** | -10.25% | -9.28% |
| **盈虧比 (Reward/Risk)** | **5.42** | **6.38** |

## 情景分析 (Scenario Analysis - Based on Quartiles)
| 情景 (Scenario) | 機率分界 (Percentile) | 預期回報率 (ROI) |
| :--- | :--- | :--- |
| **樂觀情景 (Optimistic)** | Top 25% (Q3) | > **73.95%** |
| **基準情景 (Baseline)** | Median (Q2) | **32.24%** |
| **悲觀情景 (Pessimistic)** | Bottom 25% (Q1) | < **0.85%** |
...
| :--- | :--- | :--- |
| **標準策略 (基於平均值)** | `71.41%` | `35.71%` |
| **穩健策略 (基於中位數)** | `72.08%` | `36.04%` |
| **保守策略 (基於保守預期)** | `59.38%` | `29.69%` |
| **激進策略 (基於樂觀預期)** | `74.50%` | `(不建議)` |
