...
**分析進場區間**: 2017-08-17 至 2025-09-09

## 統計概覽 (Statistics Overview)
| 指標 | 數值 | 說明 |
| :--- | :--- | :--- |
| **總交易樣本數** | 2946 天 | 模擬在此區間內每天買入 |
| **勝率 (Win Rate)** | **53.87%** (1587 天) | 持有 365 天後獲利的機率 |
| **賠率 (Loss Rate)** | 46.13% (1359 天) | 持有 365 天後虧損的機率 |
...
| **基準情景 (Baseline)** | Median (Q2) | **11.17%** |
| **悲觀情景 (Pessimistic)** | Bottom 25% (Q1) | < **-34.81%** |
...
| :--- | :--- | :--- |
| **標準策略 (基於平均值)** | `46.76%` | `23.38%` |
| **穩健策略 (基於中位數)** | `31.11%` | `15.55%` |
| **保守策略 (基於保守預期)** | `-4.03%` | `-2.02%` |
| **激進策略 (基於樂觀預期)** | `50.63%` | `(不建議)` |
