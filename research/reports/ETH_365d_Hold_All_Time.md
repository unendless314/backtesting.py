...
**數據來源**: `ETHUSDT_1d.csv`
**完整數據範圍**: 2017-08-17 至 2026-09-08
**分析進場區間**: 2017-08-17 至 2025-09-08

## 統計概覽 (Statistics Overview)
| 指標 | 數值 | 說明 |
| :--- | :--- | :--- |
| **總交易樣本數** | 2945 天 | 模擬在此區間內每天買入 |
| **勝率 (Win Rate)** | **53.89%** (1587 天) | 持有 365 天後獲利的機率 |
| **賠率 (Loss Rate)** | 46.11% (1358 天) | 持有 365 天後虧損的機率 |
| **偏度 (Skewness)** | 2.72 | >1 代表具有「暴漲長尾」特性 |

## 回報率分析 (Returns Analysis)
| 指標 | 平均值 (Mean) | 中位數 (Robust) |
| :--- | :--- | :--- |
| **整體回報率** | 118.99% | **11.22%** |
| **獲利交易平均回報** | +254.36% | +74.96% |
| **虧損交易平均回報** | -39.20% | -36.98% |
| **盈虧比 (Reward/Risk)** | **6.49** | **2.03** |

## 情景分析 (Scenario Analysis - Based on Quartiles)
| 情景 (Scenario) | 機率分界 (Percentile) | 預期回報率 (ROI) |
| :--- | :--- | :--- |
| **樂觀情景 (Optimistic)** | Top 25% (Q3) | > **78.37%** |
| **基準情景 (Baseline)** | Median (Q2) | **11.22%** |
| **悲觀情景 (Pessimistic)** | Bottom 25% (Q1) | < **-34.79%** |
...
| :--- | :--- | :--- |
| **標準策略 (基於平均值)** | `46.78%` | `23.39%` |
| **穩健策略 (基於中位數)** | `31.14%` | `15.57%` |
| **保守策略 (基於保守預期)** | `-3.99%` | `-2.00%` |
| **激進策略 (基於樂觀預期)** | `50.65%` | `(不建議)` |
