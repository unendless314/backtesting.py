...
**數據來源**: `VOO_1d.csv`
**策略條件**: 當「過去一年報酬率」低於 `99900.00%` 時買入
**持有期間**: 252 天
...
| 指標 | 數值 | 說明 |
| :--- | :--- | :--- |
| **觸發交易天數** | 3371 天 | 佔總樣本數的 100.0% |
| **勝率 (Win Rate)** | **85.02%** (2866 天) | 買入持有 252 天後獲利的機率 |
| **賠率 (Loss Rate)** | 14.98% (505 天) | 買入持有 252 天後虧損的機率 |
| **偏度 (Skewness)** | 0.05 | >1 代表具有「暴漲長尾」特性 |

## 回報率分析 (Returns Analysis)
| 指標 | 平均值 (Mean) | 中位數 (Robust) |
| :--- | :--- | :--- |
| **整體回報率** | 13.13% | **14.20%** |
| **獲利交易平均回報** | +16.74% | +15.41% |
| **虧損交易平均回報** | -7.35% | -6.54% |
| **盈虧比 (Reward/Risk)** | **2.28** | **2.36** |

## 情景分析 (Scenario Analysis - Based on Quartiles)
| 情景 (Scenario) | 機率分界 (Percentile) | 預期回報率 (ROI) |
| :--- | :--- | :--- |
| **樂觀情景 (Optimistic)** | Top 25% (Q3) | > **20.00%** |
| **基準情景 (Baseline)** | Median (Q2) | **14.20%** |
| **悲觀情景 (Pessimistic)** | Bottom 25% (Q1) | < **6.10%** |
...
| :--- | :--- | :--- |
| **標準策略 (基於平均值)** | `78.44%` | `39.22%` |
| **穩健策略 (基於中位數)** | `78.66%` | `39.33%` |
| **保守策略 (基於保守預期)** | `69.76%` | `34.88%` |
| **激進策略 (基於樂觀預期)** | `83.28%` | `(不建議)` |
