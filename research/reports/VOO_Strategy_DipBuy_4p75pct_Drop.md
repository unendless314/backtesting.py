...
| 指標 | 數值 | 說明 |
| :--- | :--- | :--- |
| **觸發交易天數** | 943 天 | 佔總樣本數的 26.8% |
| **勝率 (Win Rate)** | **97.35%** (918 天) | 買入持有 252 天後獲利的機率 |
| **賠率 (Loss Rate)** | 2.65% (25 天) | 買入持有 252 天後虧損的機率 |
| **偏度 (Skewness)** | 1.19 | >1 代表具有「暴漲長尾」特性 |

## 回報率分析 (Returns Analysis)
| 指標 | 平均值 (Mean) | 中位數 (Robust) |
| :--- | :--- | :--- |
| **整體回報率** | 18.41% | **16.27%** |
| **獲利交易平均回報** | +19.07% | +16.60% |
| **虧損交易平均回報** | -5.70% | -2.77% |
| **盈虧比 (Reward/Risk)** | **3.35** | **6.00** |

## 情景分析 (Scenario Analysis - Based on Quartiles)
| 情景 (Scenario) | 機率分界 (Percentile) | 預期回報率 (ROI) |
| :--- | :--- | :--- |
| **樂觀情景 (Optimistic)** | Top 25% (Q3) | > **22.92%** |
| **基準情景 (Baseline)** | Median (Q2) | **16.27%** |
| **悲觀情景 (Pessimistic)** | Bottom 25% (Q1) | < **12.54%** |
...
| :--- | :--- | :--- |
| **標準策略 (基於平均值)** | `96.56%` | `48.28%` |
| **穩健策略 (基於中位數)** | `96.91%` | `48.45%` |
| **保守策略 (基於保守預期)** | `95.16%` | `47.58%` |
| **激進策略 (基於樂觀預期)** | `97.28%` | `(不建議)` |
