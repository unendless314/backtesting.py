...
**數據來源**: `BRENT_1d.csv`
**完整數據範圍**: 2007-07-30 至 2026-09-09
**分析進場區間**: 2007-07-30 至 2025-09-08

## 統計概覽 (Statistics Overview)
| 指標 | 數值 | 說明 |
| :--- | :--- | :--- |
| **總交易樣本數** | 4505 天 | 模擬在此區間內每天買入 |
| **勝率 (Win Rate)** | **49.01%** (2208 天) | 持有 252 天後獲利的機率 |
| **賠率 (Loss Rate)** | 50.99% (2297 天) | 持有 252 天後虧損的機率 |
| **偏度 (Skewness)** | 0.93 | >1 代表具有「暴漲長尾」特性 |

## 回報率分析 (Returns Analysis)
| 指標 | 平均值 (Mean) | 中位數 (Robust) |
| :--- | :--- | :--- |
| **整體回報率** | 5.26% | **-0.66%** |
| **獲利交易平均回報** | +34.53% | +27.03% |
| **虧損交易平均回報** | -22.88% | -18.91% |
| **盈虧比 (Reward/Risk)** | **1.51** | **1.43** |

## 情景分析 (Scenario Analysis - Based on Quartiles)
| 情景 (Scenario) | 機率分界 (Percentile) | 預期回報率 (ROI) |
| :--- | :--- | :--- |
| **樂觀情景 (Optimistic)** | Top 25% (Q3) | > **26.03%** |
| **基準情景 (Baseline)** | Median (Q2) | **-0.66%** |
| **悲觀情景 (Pessimistic)** | Bottom 25% (Q1) | < **-19.44%** |
...
| :--- | :--- | :--- |
| **標準策略 (基於平均值)** | `15.22%` | `7.61%` |
| **穩健策略 (基於中位數)** | `13.34%` | `6.67%` |
| **保守策略 (基於保守預期)** | `-146.67%` | `-73.33%` |
| **激進策略 (基於樂觀預期)** | `40.82%` | `(不建議)` |

## 分析說明與風險提示
本報告基於歷史數據進行回測，模擬在「分析進場區間」內的**每一天**都買入該資產，並嚴格持有 **252 天**後的結果。
