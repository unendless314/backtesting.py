...
**分析進場區間**: 2021-09-10 至 2025-09-08

## 統計概覽 (Statistics Overview)
| 指標 | 數值 | 說明 |
| :--- | :--- | :--- |
| **總交易樣本數** | 1460 天 | 模擬在此區間內每天買入 |
| **勝率 (Win Rate)** | **59.52%** (869 天) | 持有 365 天後獲利的機率 |
| **賠率 (Loss Rate)** | 40.48% (591 天) | 持有 365 天後虧損的機率 |
| **偏度 (Skewness)** | 0.91 | >1 代表具有「暴漲長尾」特性 |

## 回報率分析 (Returns Analysis)
| 指標 | 平均值 (Mean) | 中位數 (Robust) |
| :--- | :--- | :--- |
| **整體回報率** | 32.00% | **6.96%** |
| **獲利交易平均回報** | +69.00% | +58.96% |
| **虧損交易平均回報** | -22.39% | -21.81% |
| **盈虧比 (Reward/Risk)** | **3.08** | **2.70** |

## 情景分析 (Scenario Analysis - Based on Quartiles)
| 情景 (Scenario) | 機率分界 (Percentile) | 預期回報率 (ROI) |
| :--- | :--- | :--- |
| **樂觀情景 (Optimistic)** | Top 25% (Q3) | > **76.95%** |
| **基準情景 (Baseline)** | Median (Q2) | **6.96%** |
| **悲觀情景 (Pessimistic)** | Bottom 25% (Q1) | < **-17.56%** |
...
| :--- | :--- | :--- |
| **標準策略 (基於平均值)** | `46.38%` | `23.19%` |
| **穩健策略 (基於中位數)** | `44.55%` | `22.27%` |
| **保守策略 (基於保守預期)** | `-29.43%` | `-14.72%` |
| **激進策略 (基於樂觀預期)** | `55.53%` | `(不建議)` |
