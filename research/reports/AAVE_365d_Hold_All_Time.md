...
**完整數據範圍**: 2020-10-15 至 2026-09-08
**分析進場區間**: 2020-10-15 至 2025-09-08

## 統計概覽 (Statistics Overview)
| 指標 | 數值 | 說明 |
| :--- | :--- | :--- |
| **總交易樣本數** | 1790 天 | 模擬在此區間內每天買入 |
| **勝率 (Win Rate)** | **47.82%** (856 天) | 持有 365 天後獲利的機率 |
| **賠率 (Loss Rate)** | 52.18% (934 天) | 持有 365 天後虧損的機率 |
| **偏度 (Skewness)** | 3.25 | >1 代表具有「暴漲長尾」特性 |
...
| **虧損交易平均回報** | -53.25% | -56.84% |
| **盈虧比 (Reward/Risk)** | **2.38** | **1.53** |

## 情景分析 (Scenario Analysis - Based on Quartiles)
| 情景 (Scenario) | 機率分界 (Percentile) | 預期回報率 (ROI) |
| :--- | :--- | :--- |
| **樂觀情景 (Optimistic)** | Top 25% (Q3) | > **81.61%** |
| **基準情景 (Baseline)** | Median (Q2) | **-10.97%** |
| **悲觀情景 (Pessimistic)** | Bottom 25% (Q1) | < **-58.71%** |
...
| :--- | :--- | :--- |
| **標準策略 (基於平均值)** | `25.87%` | `12.93%` |
| **穩健策略 (基於中位數)** | `13.64%` | `6.82%` |
| **保守策略 (基於保守預期)** | `-28.55%` | `-14.28%` |
| **激進策略 (基於樂觀預期)** | `35.18%` | `(不建議)` |
