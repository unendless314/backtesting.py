...
**分析進場區間**: 2017-09-01 至 2025-09-08

## 統計概覽 (Statistics Overview)
| 指標 | 數值 | 說明 |
| :--- | :--- | :--- |
| **總交易樣本數** | 2930 天 | 模擬在此區間內每天買入 |
| **勝率 (Win Rate)** | **64.68%** (1895 天) | 持有 365 天後獲利的機率 |
| **賠率 (Loss Rate)** | 35.32% (1035 天) | 持有 365 天後虧損的機率 |
| **偏度 (Skewness)** | 2.51 | >1 代表具有「暴漲長尾」特性 |

## 回報率分析 (Returns Analysis)
| 指標 | 平均值 (Mean) | 中位數 (Robust) |
| :--- | :--- | :--- |
| **整體回報率** | 75.77% | **39.93%** |
| **獲利交易平均回報** | +137.13% | +95.70% |
| **虧損交易平均回報** | -36.58% | -36.14% |
| **盈虧比 (Reward/Risk)** | **3.75** | **2.65** |

## 情景分析 (Scenario Analysis - Based on Quartiles)
| 情景 (Scenario) | 機率分界 (Percentile) | 預期回報率 (ROI) |
| :--- | :--- | :--- |
| **樂觀情景 (Optimistic)** | Top 25% (Q3) | > **122.98%** |
| **基準情景 (Baseline)** | Median (Q2) | **39.93%** |
| **悲觀情景 (Pessimistic)** | Bottom 25% (Q1) | < **-22.26%** |
...
| :--- | :--- | :--- |
| **標準策略 (基於平均值)** | `55.25%` | `27.63%` |
| **穩健策略 (基於中位數)** | `51.34%` | `25.67%` |
| **保守策略 (基於保守預期)** | `22.18%` | `11.09%` |
| **激進策略 (基於樂觀預期)** | `60.32%` | `(不建議)` |
