...
**數據來源**: `BTCUSDT_1d.csv`
**策略條件**: 當「過去一年報酬率」低於 `-22.28%` 時買入
**持有期間**: 365 天
**分析進場區間**: 2018-09-01 至 2025-09-09

## 統計概覽 (Statistics Overview)
| 指標 | 數值 | 說明 |
| :--- | :--- | :--- |
| **觸發交易天數** | 582 天 | 佔總樣本數的 22.7% |
| **勝率 (Win Rate)** | **87.29%** (508 天) | 買入持有 365 天後獲利的機率 |
| **賠率 (Loss Rate)** | 12.71% (74 天) | 買入持有 365 天後虧損的機率 |
| **偏度 (Skewness)** | 0.16 | >1 代表具有「暴漲長尾」特性 |

## 回報率分析 (Returns Analysis)
| 指標 | 平均值 (Mean) | 中位數 (Robust) |
| :--- | :--- | :--- |
| **整體回報率** | 79.77% | **74.77%** |
| **獲利交易平均回報** | +95.23% | +91.45% |
| **虧損交易平均回報** | -26.37% | -26.64% |
| **盈虧比 (Reward/Risk)** | **3.61** | **3.43** |

## 情景分析 (Scenario Analysis - Based on Quartiles)
| 情景 (Scenario) | 機率分界 (Percentile) | 預期回報率 (ROI) |
| :--- | :--- | :--- |
| **樂觀情景 (Optimistic)** | Top 25% (Q3) | > **131.02%** |
| **基準情景 (Baseline)** | Median (Q2) | **74.77%** |
| **悲觀情景 (Pessimistic)** | Bottom 25% (Q1) | < **34.28%** |
...
| :--- | :--- | :--- |
| **標準策略 (基於平均值)** | `83.76%` | `41.88%` |
| **穩健策略 (基於中位數)** | `83.58%` | `41.79%` |
| **保守策略 (基於保守預期)** | `76.78%` | `38.39%` |
| **激進策略 (基於樂觀預期)** | `85.53%` | `(不建議)` |
