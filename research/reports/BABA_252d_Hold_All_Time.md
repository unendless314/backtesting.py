...
**指定進場區間**: 全歷史
**實際有效進場範圍**: 2014-09-19 至 2025-09-09（持有 252 天，末端不足持有期的進場已捨棄）

## 統計概覽 (Statistics Overview)
| 指標 | 數值 | 說明 |
| :--- | :--- | :--- |
| **總交易樣本數** | 2759 天 | 模擬在此區間內每天買入 |
| **勝率 (Win Rate)** | **54.55%** (1505 天) | 持有 252 天後獲利的機率 |
| **賠率 (Loss Rate)** | 45.42% (1253 天) | 持有 252 天後虧損的機率 |
| **偏度 (Skewness)** | 0.30 | >1 代表具有「暴漲長尾」特性 |

## 回報率分析 (Returns Analysis)
| 指標 | 平均值 (Mean) | 中位數 (Robust) |
| :--- | :--- | :--- |
| **整體回報率** | 12.20% | **5.23%** |
| **獲利交易平均回報** | +42.21% | +40.69% |
| **虧損交易平均回報** | -23.85% | -19.14% |
| **盈虧比 (Reward/Risk)** | **1.77** | **2.13** |

## 情景分析 (Scenario Analysis - Based on Quartiles)
| 情景 (Scenario) | 機率分界 (Percentile) | 預期回報率 (ROI) |
| :--- | :--- | :--- |
| **樂觀情景 (Optimistic)** | Top 25% (Q3) | > **44.87%** |
| **基準情景 (Baseline)** | Median (Q2) | **5.23%** |
| **悲觀情景 (Pessimistic)** | Bottom 25% (Q1) | < **-16.90%** |
...
| :--- | :--- | :--- |
| **標準策略 (基於平均值)** | `28.90%` | `14.45%` |
| **穩健策略 (基於中位數)** | `33.18%` | `16.59%` |
| **保守策略 (基於保守預期)** | `-63.05%` | `-31.53%` |
| **激進策略 (基於樂觀預期)** | `48.39%` | `(不建議)` |

## 分析說明與風險提示
本報告基於歷史數據進行回測，模擬在「實際有效進場範圍」內的**每一天**都買入該資產，並嚴格持有 **252 天**後的結果。
