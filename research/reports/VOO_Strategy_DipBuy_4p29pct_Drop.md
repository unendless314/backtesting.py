...
**數據來源**: `VOO_1d.csv`
**策略條件**: 當「過去一年報酬率」低於 `4.29%` 時買入
**持有期間**: 252 天
**分析進場區間**: 2011-09-08 至 2025-01-17

## 統計概覽 (Statistics Overview)
| 指標 | 數值 | 說明 |
| :--- | :--- | :--- |
| **觸發交易天數** | 895 天 | 佔總樣本數的 26.6% |
| **勝率 (Win Rate)** | **97.54%** (873 天) | 買入持有 252 天後獲利的機率 |
| **賠率 (Loss Rate)** | 2.46% (22 天) | 買入持有 252 天後虧損的機率 |
| **偏度 (Skewness)** | 1.33 | >1 代表具有「暴漲長尾」特性 |

## 回報率分析 (Returns Analysis)
| 指標 | 平均值 (Mean) | 中位數 (Robust) |
| :--- | :--- | :--- |
| **整體回報率** | 18.37% | **16.28%** |
| **獲利交易平均回報** | +18.97% | +16.59% |
| **虧損交易平均回報** | -5.54% | -2.83% |
| **盈虧比 (Reward/Risk)** | **3.42** | **5.87** |

## 情景分析 (Scenario Analysis - Based on Quartiles)
| 情景 (Scenario) | 機率分界 (Percentile) | 預期回報率 (ROI) |
| :--- | :--- | :--- |
| **樂觀情景 (Optimistic)** | Top 25% (Q3) | > **22.63%** |
| **基準情景 (Baseline)** | Median (Q2) | **16.28%** |
| **悲觀情景 (Pessimistic)** | Bottom 25% (Q1) | < **12.63%** |
...
| :--- | :--- | :--- |
| **標準策略 (基於平均值)** | `96.82%` | `48.41%` |
| **穩健策略 (基於中位數)** | `97.12%` | `48.56%` |
| **保守策略 (基於保守預期)** | `95.75%` | `47.88%` |
| **激進策略 (基於樂觀預期)** | `97.48%` | `(不建議)` |
