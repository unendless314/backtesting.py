"""AAVE DCA dashboard that mirrors the ETH logic but ingests `data/raw/AAVEUSDT_1d.csv`. """
...
DATA_PATH = Path('data/raw/AAVEUSDT_1d.csv')
...
def render_dashboard(close: pd.Series, data_map, metrics_map, intervals, output_html: Path) -> None:
    ...
    output_file(output_html, title='AAVE DCA Dashboard')
...
def main() -> None:
    ...
    output_html = RESULTS_DIR / f'aave_dca_dashboard_mvp_{datetime.now():%Y%m%d_%H%M%S}.html'
