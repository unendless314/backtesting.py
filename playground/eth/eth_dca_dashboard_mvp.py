"""ETH DCA dashboard that mirrors the VOO logic but ingests `data/raw/ETHUSDT_1d.csv`. """
...
DATA_PATH = Path('data/raw/ETHUSDT_1d.csv')
...
THRESHOLDS = [-5, -10, -15, -20, -25, -30, -35, -40, -50]  # Expanded for Crypto volatility


def find_peak_intervals(close: pd.Series, min_len: int = 30, top_n: int = 10) -> List[Tuple[str, int, int]]:
    """Return top_n longest ATH→ATH intervals (label, start_idx, end_idx).
    
    Crypto cycles are faster, so min_len reduced to 30.
    """
...
def read_price_data() -> pd.DataFrame:
    ...
    # df.index.name = 'Datetime' # Already set by set_index
    return df[['Close']]
...
def simulate_dca(
    ...
    if not returns.empty:
        ...
        if downside > 0:
            sortino = mean_ret / downside * np.sqrt(TRADING_DAYS)

    period_days = (close.index[-1] - close.index[0]).days
    if period_days <= 0:
        period_years = len(close) / TRADING_DAYS
    else:
        period_years = period_days / 365.0
    if period_years <= 0:
        period_years = 1 / TRADING_DAYS
...
def render_dashboard(close: pd.Series, data_map, metrics_map, intervals, output_html: Path) -> None:
    ...
    output_file(output_html, title='ETH DCA Dashboard')
...
def main() -> None:
    ...
    output_html = RESULTS_DIR / f'eth_dca_dashboard_mvp_{datetime.now():%Y%m%d_%H%M%S}.html'
