"""ETH ATH-to-ATH interval + drawdown viewer mirroring the VOO logic.

Run inside the repo's virtualenv:
    source .venv/bin/activate
    python playground/eth/eth_peak_drawdown_ui_1d.py --help
"""
...
DEFAULT_DATA = Path('data/raw/ETHUSDT_1d.csv')
RESULTS_DIR = Path('playground/results')
# Drawdown bands to visualize (include extreme -80% for Crypto)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description='Visualize ETH ATH gaps and their drawdown bands.',
    ...
    parser.add_argument('--title', type=str, default='ETH Peak Gap + Drawdown',
...
def read_price_data(csv_path: Path) -> pd.DataFrame:
    ...
    # Crypto CSV usually has 'datetime'
    ...
    # Ensure columns exist (Crypto CSVs usually have lowercase open, high...)
...
def main() -> None:
    ...
    if interval_targets:
        ...
        for label, row in interval_targets:
            ...
            outfile = RESULTS_DIR / f"eth_peak_gap_dd_{suffix}_{batch_tag}.html"
            output_file(outfile)
            save(layout)
    ...
    outfile = RESULTS_DIR / f'eth_peak_gap_dd_{timestamp}.html'
    output_file(outfile)
    save(layout)
