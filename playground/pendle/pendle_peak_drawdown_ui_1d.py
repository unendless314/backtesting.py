"""Pendle ATH-to-ATH interval + drawdown viewer (only one major bear phase expected).

Run inside the repo's virtualenv:
    source .venv/bin/activate
    python playground/pendle/pendle_peak_drawdown_ui_1d.py --help
"""
...
DEFAULT_DATA = Path('data/raw/PENDLE.csv')
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description='Visualize Pendle ATH gaps and their drawdown bands.',
    ...
    parser.add_argument('--title', type=str, default='Pendle Peak Gap + Drawdown',
...
def main() -> None:
    ...
    if interval_targets:
        ...
        for label, row in interval_targets:
            ...
            outfile = RESULTS_DIR / f"pendle_peak_gap_dd_{suffix}_{batch_tag}.html"
            output_file(outfile)
            save(layout)
    ...
    outfile = RESULTS_DIR / f'pendle_peak_gap_dd_{timestamp}.html'
    output_file(outfile)
    save(layout)
