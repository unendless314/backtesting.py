...
I coded the repeated DCA scenarios in `playground/dca_simulation.py`. Each run waits for a drawdown of `-50%` or worse, then buys the same dollar slice each day for the configured span (1 – 1,000 days) or until the deep drawdown resolves. Results after the full data sweep:
