"""VOO DCA dashboard that mirrors the Pendle logic but ingests `data/raw/VOO.csv`. """
...
DATA_PATH = Path('data/raw/VOO.csv')
...
def read_price_data() -> pd.DataFrame:
    ...
    return df[df.index >= '2010-09-09'][['Close']]
...
def main() -> None:
    ...
    output_html = RESULTS_DIR / f'voo_dca_dashboard_mvp_{datetime.now():%Y%m%d_%H%M%S}.html'
