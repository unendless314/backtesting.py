"""VOO ATH-to-ATH interval + drawdown viewer mirroring the Pendle logic.

Run inside the repo's virtualenv:
    source .venv/bin/activate
    python playground/voo/voo_peak_drawdown_ui_1d.py --help
"""
...
DEFAULT_DATA = Path('data/raw/VOO.csv')
RESULTS_DIR = Path('playground/results')
# Drawdown bands to visualize (exclude extreme -80% per user request)
DD_LEVELS = [-10, -20, -30, -40, -50, -60, -70]
DD_COLORS = ['#f5c6a5', '#f2a65a', '#d65f5f', '#c44a5c', '#a63d5f', '#7f2f5d', '#5b1f5b']


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description='Visualize VOO ATH gaps and their drawdown bands.',
    ...
    parser.add_argument('--start', type=str, default='2010-09-09',
    ...
    parser.add_argument('--title', type=str, default='VOO Peak Gap + Drawdown',
    ...
    parser.add_argument('--interval-padding-days', type=int, default=60,
...
def _dd_histogram_html(drawdown: pd.Series) -> str:
    ...
    # Bins must be ascending; include low sentinel to cover deep tails
    edges = [-1_000_000, -70, -60, -50, -40, -30, -20, -10, 0]
    labels = [
        '<-70%',
        '-70% to -60%',
        '-60% to -50%',
...
def main() -> None:
    ...
    if interval_targets:
        ...
        for label, row in interval_targets:
            ...
            outfile = RESULTS_DIR / f"voo_peak_gap_dd_{suffix}_{batch_tag}.html"
            output_file(outfile)
            save(layout)
    ...
    outfile = RESULTS_DIR / f'voo_peak_gap_dd_{timestamp}.html'
    output_file(outfile)
    save(layout)
